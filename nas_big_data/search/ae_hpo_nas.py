...
class AeHpoNas(RegularizedEvolution):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # probability of mutating the architecture
        self.p = len(self.space_list) / (len(self.hp_space) + len(self.space_list))

    @staticmethod
    def _extend_parser(parser):
        RegularizedEvolution._extend_parser(parser)
        add_arguments_from_signature(parser, AeHpoNas)
        return parser
    ...
    def gen_random_batch(self, size: int) -> list:
        batch = []
        iterator = zip(*(self.hp_space[i].rvs(n_samples=size) for i in range(3)))
        ...
        for sample in iterator:
            ...
            # hyperparameters
            cfg["hyperparameters"]["learning_rate"] = sample[0]
            cfg["hyperparameters"]["batch_size"] = sample[1]
            cfg["hyperparameters"]["ranks_per_node"] = sample[2]
    ...
    def copy_mutate_arch(self, parent_cfg: dict) -> dict:
        ...
        child_cfg = parent_cfg.copy()
        child_arch = child_cfg["arch_seq"]

        mutate_arch = np.random.binomial(1, self.p) == 1

        if mutate_arch:

            i = np.random.choice(len(self.space_list))
            ...
            child_arch[i] = sample
            child_cfg["arch_seq"] = child_arch
        else:  # mutate hp
            hps = ["learning_rate", "batch_size", "ranks_per_node"]
            i = np.random.randint(0, len(hps))
            s = self.hp_space[i].rvs()
            s = s[0] if type(s) is list else s
            child_cfg["hyperparameters"][hps[i]] = s

        return child_cfg


if __name__ == "__main__":
    args = AeHpoNas.parse_args()
    search = AeHpoNas(**vars(args))
    search.main()
